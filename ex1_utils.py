...
def transformRGB2YIQ(imgRGB: np.ndarray) -> np.ndarray:
    ...
    #convert to shape we can multiply with the YIQ kernel matrix :
...
def transformYIQ2RGB(imgYIQ: np.ndarray) -> np.ndarray:
    ...
    #linalg.inv - Compute the (multiplicative) inverse of a matrix. on that we do transpose and then multiply with
    # the YIQ image 
